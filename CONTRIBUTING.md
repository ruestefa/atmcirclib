...
3. Create a virtual environment and install the runtime and development dependencies:
    ...
    This will create a conda environment named `atmcirclib` (change with `-n`) and install the following:
...
- `tools/`: Scripts primarily for development
    ...
    - `create_envs.sh`: Create conda environment from `environment.yml` or `requirements.yml`.
    ...
    - `update_min_reqs.sh`: Set the minimum versions of all packages in `requirements.yml` to the minor version in `environment.yml`.
...
- `environment.yml`: Complete tree of runtime and development dependencies with fully pinned version numbers; created with `conda env export`.
...
- `requirements.yml`: Top-level runtime and development dependencies with only necessary version restrictions (typically a minimum version or a version range); kept manually.
- `USAGE.md`: Information on how to use the package.

## Managing dependencies

AtmCircLib uses [Conda](https://docs.conda.io/en/latest/) to manage dependencies. (Also check out [Mamba](https://mamba.readthedocs.io/en/latest/) if you like your package installations fast.) Dependencies are specified in YAML files, of which there are two:

- `requirements.yml`: Top-level runtime and development dependencies with minimal version restrictions (typically a minimum version or a version range). It is kept manually.
- `environment.yml`: Full tree of runtime and development dependencies with fully specified ('pinned') version numbers. It is created with `conda env export`.
